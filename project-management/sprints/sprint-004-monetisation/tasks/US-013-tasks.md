...
| T-013-09 | [FE-WEB] | Mise à jour `templates/brief/index.html.twig` : intercepter HTTP 402 (quota_exceeded) pour afficher `paywall-modal` via `GET /quota/paywall-modal` ; intercepter HTTP 503 séparément (message d'erreur service) ; `templates/quota/_quota_indicator.html.twig` inclus dans header | 1h | T-013-08 | ✅ |
...
| T-013-12 | [TEST] | `WebTestCase` : `GET /api/v1/quota` → JSON `{used, limit, remaining}` + `X-Quota-Remaining` header correct ; route `/quota/paywall-modal` : CTA actif sans `aria-disabled`, sans `href="#"`, présence `premium_checkout` | 1.5h | T-013-05, T-013-07 | ✅ |
