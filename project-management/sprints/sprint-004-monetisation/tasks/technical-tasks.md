...
| T-TECH-02 | [OPS] | Variables d'environnement Stripe : ajout dans `env.example` (valeurs vides) des 5 variables : `STRIPE_SECRET_KEY=`, `STRIPE_PUBLISHABLE_KEY=`, `STRIPE_WEBHOOK_SECRET=`, `STRIPE_PRICE_MONTHLY=`, `STRIPE_PRICE_YEARLY=` ; ajout dans `phpunit.dist.xml` des clés test Stripe (sk_test_fake, whsec_test_xxx, price_test_xxx) via `<server>` tags ; JAMAIS de valeur réelle dans les fichiers versionnés | 0.5h | — | ✅ |
